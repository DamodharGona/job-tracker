...
const getJobApplication = async (
  ...
  const result = await getJobApplicationService(
...
const deleteJobApplication = async (
  ...
  const payload = req.user as { userId: string; email: string };
  const userId = payload.userId;
  const result = await deleteJobApplicationService(
    req.params.id as string,
    userId,
  );
...
const applicationsMetrics = async (
  req: Request,
  res: Response,
): Promise<void> => {
  const payload = req.user as { userId: string; email: string };
...
export async function jdKeyWordMatcher(
  req: Request,
  res: Response,
): Promise<void> {
  const file = req.file;
...
router.post(
  "/",
  validateRequest(createJobApplicationRequest),
...
router.get(
  "/",
  validateRequest(getAllJobApplicationsRequest),
...
router.get(
  "/:id",
  validateRequest(getJobApplicationRequest),
  getJobApplication,
);
...
router.patch(
  "/:id",
  validateRequest(updateJobApplicationRequest),
...
router.delete(
  "/:id",
  validateRequest(deleteJobApplicationRequest),
